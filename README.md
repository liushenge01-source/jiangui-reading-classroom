...
- 教师入口：https://liushenge01-source.github.io/jiangui-reading-classroom/
...
2. 在 Supabase Authentication 里创建刘老师的邮箱/密码账号，并关闭公开注册，只保留老师账号。
3. 将项目 URL 和 publishable/anon key 填入 `config.js`。此键可用于浏览器；绝不能填写 `service_role` 密钥。
4. 提交并发布 `config.js` 后，学生可以匿名提交，只有登录的老师账号可以读取作品。

学生姓名为可选昵称；建议课堂中使用昵称或匿名。学生提交的预测、依据和绘本会存入该 Supabase 项目。在线数据库接通前，提交按钮会提示尚未配置，不会假装提交成功。
